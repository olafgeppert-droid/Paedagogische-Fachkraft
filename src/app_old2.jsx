...
const App = () => {
    ...
    const handleAddStudent = async (student) => {
        ...
        try {
            ...
            setModal('student'); // Korrektur: "Neuer Schüler" öffnet nun StudentModal
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            fileInput.onchange = async (event) => {
                ...
                if (file) {
                    await importData(db, file, setSettings, setMasterData, setStudents, setModal);
                    const updatedStudents = await getStudents(db);
                    setStudents(updatedStudents);
                    setSelectedStudent(updatedStudents.length > 0 ? updatedStudents[0] : null);
                    await loadEntries();
            ...
            fileInput.click(); // Korrektur: Import öffnet jetzt Dialog
        } catch (err) {
    ...
    const handlePrint = () => {
        window.print(); // Korrektur: Drucken öffnet jetzt das Browser-Druckfenster
    };
    ...
    const handleStudentClick = (student) => {
        ...
        setModal(null); // Korrektur: Klick auf Schüler öffnet **nicht mehr** das Bearbeiten-Modal
    };

    const handleEditStudent = () => {
        if (selectedStudent) setModal('student'); // Korrektur: Schüler bearbeiten öffnet Modal
    };

    const handleOpenSearch = () => {
        setSearchModalOpen(true); // Korrektur: Protokoll suchen öffnet Modal
    };
